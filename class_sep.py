...
classes = os.listdir(data_dir + args.predict + "/train")
...
dataset = ImageFolder(data_dir + args.predict + '/train', transform = train_transform)
total = len(dataset)
    ...
    def val(self):
        ...
        with torch.no_grad():
            ...
            if epoch_acc >= self.best_acc:
                ...
                self.best_acc_wts = copy.deepcopy(self.model.state_dict())
                torch.save(self.model.state_dict(), './sep/weight/new_one/' + args.predict + '/best_model_' + args.model_name + '.pth') 
                print('Model Saved.')
...
checkpoint = torch.load('./sep/weight/new_one/' + args.predict + '/best_model_' + args.model_name + '.pth')
